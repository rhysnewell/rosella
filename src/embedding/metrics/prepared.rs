use super::{
    AggregateMetric, CoverageAggregation, DistanceSettings, EPSILON, Moments, Overlaps, finish,
    overlap, peak_mean, rho_from,
};
...
impl PreparedAggregate {
    pub fn new(
        ...
        for (row, floor) in rows.iter().zip(floors) {
            ...
            let mean = if composition.is_empty() {
                0.0
            } else {
    ...
    pub fn distance(&self, a: usize, b: usize) -> f64 {
        ...
        self.metric.combine(coverage, scored, self.rho(a, b))
    }
    ...
    fn rho(&self, a: usize, b: usize) -> f64 {
        rho_from(
