...
impl<'a> CovermEngine<'a> {
    ...
    fn run_coverm(
        ...
        coverm_command
            ...
            .arg(&format!("{}", self.settings.min_covered_fraction));
        ...
        match mode {
            MappingMode::ShortRead | MappingMode::ShortBam => {
                ...
                if let Some(min_read_aligned_length_pair) =
                    &self.settings.min_read_aligned_length_pair
                {
                    coverm_command
                ...
                if let Some(min_read_percent_identity_pair) =
                    &self.settings.min_read_percent_identity_pair
                {
                    coverm_command
                ...
                if let Some(min_read_aligned_percent_pair) =
                    &self.settings.min_read_aligned_percent_pair
                {
                    coverm_command
...
impl CovermSettings {
    pub fn new(m: &clap::ArgMatches) -> Result<Self> {
        ...
        let min_read_aligned_length_pair =
            m.get_one::<u32>("min-read-aligned-length-pair").cloned();
        let min_read_percent_identity_pair =
            m.get_one::<f32>("min-read-percent-identity-pair").cloned();
        let min_read_aligned_percent_pair =
            m.get_one::<f32>("min-read-aligned-percent-pair").cloned();
